...
# Plots
plot_font = 18
common = (; xlims=(t_cas[1]-0.01, t_cas[end]), tickfontsize=plot_font,
    guidefontsize=plot_font, legendfontsize=plot_font)
p1 = plot(t_cl, s22_cl; color=:blue, label=L"\mathrm{classical}",
    ylabel=L"P", xticks=([0, 10, 20], ["", "", ""]),
    yticks=([0, 0.5, 1], [L"0.0", L"0.5", L"1.0"]), legend=:topleft, common...)
plot!(p1, t_cas, real.(s22_cas); color=:red, ls=:dash, label=L"\mathrm{cascade}")
p2 = plot(t_cas, nu1_cas; color=:blue, label=L"\langle \hat{n}_{u_1} \rangle",
    ylabel=L"\langle\hat{n}_i\rangle", xticks=([0, 10, 20], ["", "", ""]),
    yticks=([0, 5e3, 10e3, 15e3], [L"0", L"5\cdot10^3", L"10\cdot10^3", L"15\cdot10^3"]),
    ylims=(nv1_cas[1]-0.5e3, nu1_cas[1]+0.5e3), legend=:left, common...)
plot!(p2, t_cas, nv1_cas; color=:red, label=L"\langle \hat{n}_{v_1} \rangle")
plot!(p2, t_cas, nu2_cas; color=:blue, ls=:dash, label=L"\langle \hat{n}_{u_2} \rangle")
plot!(p2, t_cas, nv2_cas; color=:red, ls=:dash, label=L"\langle \hat{n}_{v_2} \rangle")
p3 = plot(t_cas, nu1_cas .+ nv1_cas .- nu1_cas[1]; color=:blue, label=L"\mathrm{mode~1}",
    ylabel=L"\langle\Delta\hat{n}_i\rangle", xlabel=L"\gamma t",
    xticks=([0, 10, 20], latexstring.([0, 10, 20])),
    yticks=([-4, -2, 0, 2, 4], latexstring.([-4, -2, 0, 2, 4])),
    legend=:topleft, common...)
...
eqs_int = meanfield(ops, H_int_sym_t, [L_int_sym_t]; Jdagger=[Ld_int_sym_t], order=order, iv=t);
...
nu2_df = real.(expect(embed(b,1,(anu2 + α2_io*identityoperator(bu2))'*(anu2 + α2_io*identityoperator(bu2))), ρt_df))
nu1_df = real.(expect(embed(b,2,(anu1 + α1_io*identityoperator(bu1))'*(anu1 + α1_io*identityoperator(bu1))), ρt_df))
...
pl4 = plot(t_int, nu1_int .+ nv1_int .- nu1_int[1]; color=:blue, label=L"\mathrm{mode~1~(int.)}",
    ylabel=L"\langle\Delta\hat{n}_i\rangle", xlabel=L"\gamma t",
    xlims=(t_df[1]-0.01, t_df[end]), yticks=([-2, -1, 0, 1], latexstring.([-2, -1, 0, 1])),
    legend=:right, tickfontsize=plot_font, guidefontsize=plot_font,
    legendfontsize=plot_font, size=(800, 400))
plot!(pl4, t_int, nu2_int .+ nv2_int .- nu2_int[1]; color=:red, label=L"\mathrm{mode~2~(int.)}")
plot!(pl4, t_df, nu1_df .+ nv1_df .- nu1_df[1]; color=:green, ls=:dash, label=L"\mathrm{mode~1~(dis.)}")
plot!(pl4, t_df, nu2_df .+ nv2_df .- nu2_df[1]; color=:orange, ls=:dash, label=L"\mathrm{mode~2~(dis.)}")
display(pl4)
...
order = 1 
ops = [au1, au2, s(2,2), s(2,1), av1, av2] 
eqs_con = meanfield(ops, Hcon_t, [Lch1_t, Lch2_t]; Jdagger=[Ldch1_t,Ldch2_t], order=order, iv=t)
...
common = (; xlims=(t_con[1]-0.01, t_con[end]), tickfontsize=plot_font,
    guidefontsize=plot_font, legendfontsize=plot_font)
p1 = plot(t_cl, real.(s22_cl); color=:blue, label=L"\mathrm{cascade}", ylabel=L"P",
    xticks=([0, 10, 20], ["", "", ""]), legend=:topleft, common...)
plot!(p1, t_con, real.(s22_con); color=:red, ls=:dash, label=L"\mathrm{concat.}")
p2 = plot(t_con, nu1_con; color=:blue, label=L"\langle \hat{n}_{u_1} \rangle",
    ylabel=L"\langle\hat{n}_i\rangle", xticks=([0, 10, 20], ["", "", ""]),
    ylims=(nv1_con[1]-0.5e3, nu1_con[1]+0.5e3), legend=:left, common...)
plot!(p2, t_con, nv1_con; color=:red, label=L"\langle \hat{n}_{v_1} \rangle")
plot!(p2, t_con, nu2_con; color=:blue, ls=:dash, label=L"\langle \hat{n}_{u_2} \rangle")
plot!(p2, t_con, nv2_con; color=:red, ls=:dash, label=L"\langle \hat{n}_{v_2} \rangle")
p3 = plot(t_con, nu1_con .+ nv1_con .- nu1_con[1]; color=:blue, label=L"\mathrm{mode~1}",
    ylabel=L"\langle\Delta\hat{n}_i\rangle", xlabel=L"\gamma t",
    yticks=([-2, -1, 0, 1, 2], latexstring.([-2, -1, 0, 1, 2])),
    legend=:bottomleft, common...)
...
eqs_1m = meanfield(ops_1m, Hcas_1m_t, [Lcas_1m_t]; Jdagger=[Lcas_1md_t], order=order, iv=t);
complete!(eqs_1m);
length(eqs_1m)
...
common = (; xlims=(t_1m[1]-0.01, t_1m[end]), tickfontsize=plot_font,
    guidefontsize=plot_font, legendfontsize=plot_font)
p1 = plot(t_cl, s22_cl; color=:blue, label=L"\mathrm{classical}", ylabel=L"P",
    xticks=([0, 10, 20], ["", "", ""]), legend=:topleft, common...)
plot!(p1, t_1m, real.(s22_1m); color=:red, ls=:dash, label=L"\mathrm{single~SUPER}")
p2 = plot(t_1m, nu_1m; color=:blue, label=L"\langle \hat{n}_{u} \rangle",
    ylabel=L"\langle\hat{n}_i\rangle", xticks=([0, 10, 20], ["", "", ""]),
    ylims=(nv_1m[1]-0.5e3, nu_1m[1]+0.5e3), legend=:left, common...)
plot!(p2, t_1m, nv_1m; color=:red, label=L"\langle \hat{n}_{v} \rangle")
p3 = plot(t_1m, nu_1m .+ nv_1m .- nu_1m[1]; color=:blue, label=L"\mathrm{mode}",
    ylabel=L"\langle\Delta\hat{n}_i\rangle", xlabel=L"\gamma t",
...
eqs_1m0 = meanfield(ops_1m0, Hcas_1m0_t, [Lcas_1m0_t]; Jdagger=[Lcas_1m0d_t], order=order, iv=t);
complete!(eqs_1m0);
length(eqs_1m0)
...
Corrmat = [[CorrelationFunction(Lvec[k]', Lvec[l], eqs_1m0; steady_state = false, iv0=t) for l = 1:length(Lvec)] for k = 1:length(Lvec)]
...
function compute_Gmat(sol_1m0, eqs_1m0, Lvec, Corrvec; p0, ps, saveat, τ0=0.0,
    ...
    padleftN(v::AbstractVector{<:Number}, N::Int) =
        (length(v) < N ? vcat(zeros(ComplexF64, N - length(v)), ComplexF64.(v)) : ComplexF64.(v))
...
for i1 = 1:N 
    for j1 = 1:N
        gcorr1[j1,i1] = gu_c_t(t1[j1])*gu_t(t1[i1])*Gmat_bar[i1,1,1,j1] + gu_c_t(t1[j1])*sqrt(γ_)*Gmat_bar[i1,1,2,j1] + gu_t(t1[i1])*sqrt(γ_)*Gmat_bar[i1,2,1,j1] + γ_*Gmat_bar[i1,2,2,j1]
    end
end
gcorr1
...
pl6 = heatmap(t1, t1, g_vis; color=:thermal, clims=(0, 1),
    xlabel=L"\gamma t_2", ylabel=L"\gamma t_1",
    colorbar_title=L"|g^{(1)}|\ \mathrm{(norm)}",
    colorbar_ticks=(tick_vals, tick_labs), aspect_ratio=:equal,
    tickfontsize=16, guidefontsize=18, size=(600, 500))
display(pl6)
...
pl7 = plot(t1, abs.(u.(t1)); color=:blue, label=L"v(t) = u(t)", ylabel=L"|v(t)|",
    xlims=(t1[1]-0.01, t1[end]), xticks=([0, 10, 20], ["", "", ""]),
    legend=:topleft, tickfontsize=plot_font, guidefontsize=plot_font,
    legendfontsize=plot_font, size=(800, 200))
...
Ω1(t_) = α1_/(sqrt(2π*σ1_^2)) *
         exp(-(t_-τ1_)^2/(2σ1_^2)) *
         exp(-1im*Δ1_*t_)

Ω2(t_) = α2_/(sqrt(2π*σ2_^2)) *
         exp(-(t_-τ2_)^2/(2σ2_^2)) *
         exp(-1im*Δ2_*t_)
...
ops_all1 = unique([ops1[i]*ops1[j] for i = 1:length(ops1) for j = 1:length(ops1)]);
ops_all = union(ops1,ops_all1[1:1],ops_all1[3:14], ops_all1[16:16], ops_all1[18:26], ops_all1[28:29])
eqs_1m = meanfield(ops_all, Hcas_1m_t, [Lcas_1m_t]; Jdagger=[Lcas_1md_t], order=order, iv=t)
...
Corrmat = [[CorrelationFunction(Lvec[k]', Lvec[l], eqs_1m; steady_state = false) for l = 1:length(Lvec)] for k = 1:length(Lvec)]
Corrvec = [Corrmat[end][i] for i = 1:length(Lvec)]
...
for i1 = 1:N 
    for j1 = 1:N
        gcorr1[i1,j1] = sum(gvecc(t1[i1])[k]*gvec(t1[j1])[l]*(Gmat_bar[j1,k,l,i1]) for k = 1:3 for l = 1:3)
    end
end
gcorr1
Λ = zeros(ComplexF64,N,N)
...
tick_vals = collect(0.0:0.2:1.0)
tick_labs = latexstring.(string.(round.(tick_vals; digits = 1)))
pl8 = heatmap(t1, t1, g_vis; color=:thermal, clims=(0, 1),
    xlabel=L"\gamma t_2", ylabel=L"\gamma t_1",
    colorbar_title=L"|g^{(1)}|\ \mathrm{(norm)}",
    colorbar_ticks=(tick_vals, tick_labs), aspect_ratio=:equal,
    tickfontsize=16, guidefontsize=18, size=(600, 500))
...
pl9 = plot(t1, abs.(u.(t1)); color=:blue, label=L"v(t) = u(t)", ylabel=L"|v(t)|",
    xlims=(t1[1]-0.01, t1[end]), xticks=([0, 10, 20], ["", "", ""]),
    legend=:topleft, tickfontsize=plot_font, guidefontsize=plot_font,
    legendfontsize=plot_font, size=(800, 200))
