...
# We calculate the coupling for the input and output cavities. The modified couplings of the second input and output modes are obtained with the function effective_input_mode and effective_output_mode, respectively. Due to the fast oscllations, the tolarance of the numeric solver needs to be improved. 
...
## Classical-to-coherent amplitude relation # TODO: adapt after α "problem" is solved
...
common = (; xlims=(t_cas[1]-0.01, t_cas[end]), tickfontsize=18,
    guidefontsize=18, legendfontsize=18)
...
a_c_int_ls = [sum(conj(M(i,j))*a0_ls[j]' for j = 1:la) for i = 1:la] # # TODO: needed?? - TEST!
...
# M-matrix
Mat  = Matrix{Any}(undef, la, la) # # TODO: rename
...
# Expectation values
t_int = sol_int.t
s22_int = real.(solution_values(sol_int, s(2,2), eqs_int))
...
pl4 = plot(t_int, nu1_int .- nu1_int[1]; color=:blue, label=L"\mathrm{mode~1~(int.)}",
    ylabel=L"\langle\Delta\hat{n}_i\rangle", xlabel=L"\gamma t",
    xlims=(t_int[1]-0.01, t_int[end]), yticks=([-2, -1, 0, 1], latexstring.([-2, -1, 0, 1])),
    legend=:right, tickfontsize=18, guidefontsize=18,
    legendfontsize=18, size=(800, 400))
...
n1_fock = round(Int, abs2(α1_io))
n2_fock = round(Int, abs2(α2_io))
...
common = (; xlims=(t_fock[1]-0.01, t_fock[end]), tickfontsize=18,
    guidefontsize=18, legendfontsize=18)
...
p3_2 = plot(t_fock, nu1_fock .- nu1_fock[1]; color=:green, label=L"\mathrm{Fock: mode~1}",
    ylabel=L"\langle\Delta\hat{n}_i\rangle", xlabel=L"\gamma t",
    xlims=(t_fock[1]-0.01, t_fock[end]), yticks=([-2, -1, 0, 1], latexstring.([-2, -1, 0, 1])),
    legend=:right, common...)
plot!(p3_2, t_fock, nu2_fock .- nu2_fock[1]; color=:yellow, label=L"\mathrm{Fock: mode~2}")
plot!(p3_2, t_int, nu1_int .- nu1_int[1]; color=:blue, label=L"\mathrm{Coherent: mode~1}")
plot!(p3_2, t_int, nu2_int .- nu2_int[1]; color=:red, label=L"\mathrm{Coherent: mode~2}")
